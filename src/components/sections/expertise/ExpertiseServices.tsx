      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {expertiseServices.map((service) => (
            <div
              key={service.id}
              className="group relative bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-[#DAEEF8] hover:border-[#E8630A]/30"
            >
              <div className="h-1.5 bg-gradient-to-r from-[#E8630A] via-[#4DA6D9] to-[#1A2940]" />

              <div className="p-6">
                <div className="relative mb-6">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#1A2940] to-[#0D5A8A] flex items-center justify-center text-white group-hover:scale-110 transition-transform duration-500 shadow-lg">
                    {iconMap[service.icon] || <Zap className="h-6 w-6" />}
                  </div>
                  <div className="absolute inset-0 w-14 h-14 rounded-2xl bg-[#E8630A]/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                </div>

                <h3 className="text-lg font-bold text-[#1A2940] mb-2 group-hover:text-[#1A2940] transition-colors line-clamp-2">
                  {service.title}
                </h3>
                ...
                <ul className="space-y-2 mb-6">
                  {service.features.slice(0, 3).map((feature, i) => (
                    <li key={i} className="flex items-center gap-2 text-sm text-[#4A6580]">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#E8630A]" />
                      {feature}
                    </li>
                  ))}
                </ul>
                ...
                <Button
                  asChild
                  variant="ghost"
                  className="w-full justify-between text-[#1A2940] hover:text-[#1A2940] hover:bg-[#E8630A]/10 group/btn"
                >
                  <Link href="/contact">
                    En savoir plus
