...
export function HomeADN() {
  return (
    <section className="py-24 bg-[#001a33] relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-[#001a33] via-[#002244] to-[#001a33] opacity-50" />

      <div className="container mx-auto px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            <span className="inline-block px-4 py-1 rounded-full bg-[#EC8D19]/10 text-[#EC8D19] text-sm font-semibold uppercase tracking-wider">
            ...
            <div className="space-y-6 pt-4">
              {steps.map((step, i) => (
                <div key={i} className="flex items-start gap-4 group">
                  <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-[#EC8D19]/10 flex items-center justify-center group-hover:bg-[#EC8D19] transition-colors duration-300">
                    <step.icon className="w-6 h-6 text-[#EC8D19] group-hover:text-white transition-colors duration-300" />
                  </div>
                  <div>
            ...
            <Button
              asChild
              size="lg"
              className="mt-8 bg-white text-[#001a33] hover:bg-[#EC8D19] hover:text-white text-lg px-8 py-6 font-semibold rounded-xl transition-all duration-300"
            >
              <Link href="/expertise">
