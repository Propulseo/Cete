...
function PortailContent({ overrides }: { overrides: PageOverridesMap }) {
  ...
  const rt = (key: string) => resolveText(overrides, `form.${key}`, t(key), locale as "fr" | "en");
  const { login } = useAuth();
  ...
  return (
    <div className="cete-login">
      <main className="shell">
        <LoginBrandPanel overrides={overrides} />
...
export function PortailPageClient({ overrides }: { overrides: PageOverridesMap }) {
  return (
    <AuthProvider>
      <PortailContent overrides={overrides} />
    </AuthProvider>
  );
}
