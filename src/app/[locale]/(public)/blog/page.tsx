...
export default async function BlogPage({
  ...
  const [posts, overrides] = await Promise.all([
    loadPublishedArticles(locale as Locale),
    loadPageOverrides("blog"),
  ]);
  ...
  return (
    <>
      <BlogHero count={posts.length} overrides={overrides} locale={locale as "fr" | "en"} />
      {featured && <BlogFeatured post={featured} overrides={overrides} />}
      {others.length > 0 && <BlogFilter posts={others} />}
      <BlogCTA overrides={overrides} />
    </>
  );
}
